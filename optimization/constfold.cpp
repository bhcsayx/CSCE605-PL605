...
void constFold::run(SSABuilder builder) {
    ...
    while(map_iter != builder.blocks.end()) {
        ...
        for(auto blk: *(builder.blocks[funcName])) {
            for(auto ins: blk->instructions) {
                eliminated[ins] = NULL;
                if(ins->op1->type == Type::constVal && ins->op2->type == Type::constVal) {
                    if(ins->opcode == OpCode::ADD) {
                        Instruction* _new = new Instruction();
                        _new->opcode = OpCode::MOVE;
                        _new->dest = ins->dest;
                        _new->op1 = new Value();
                        _new->op1->value = ins->op1->value + ins->op2->value;
...
void constFold::addInstructionwithRes(Dot* dot, string name, string funcName, Instruction* ins) {
    string res;

    if(ins->opcode != OpCode::PHI) {
        if(eliminated[ins] != NULL)
    ...
    if(eliminated[ins] != NULL)
        addInstructionwithRes(dot, name, funcName, eliminated[ins]);
}

void constFold::dump(SSABuilder builder, string name) {
    Dot dot;
